...
export default function OnScan({ route, navigation }) {
  ...
  const getDataOfNHM = async (data) => {
    try {
      ...
      console.log("http://localhost:44369/api/BacSiRules/XacNhanNhanQua?uid="+data.split("|")[0] + "&idsk="+data.split("|")[1]);
      const res_info_data = await axios.get(
        "http://localhost:44369/api/BacSiRules/XacNhanNhanQua?uid="+data.split("|")[0] + "&idsk="+data.split("|")[1],
        config
