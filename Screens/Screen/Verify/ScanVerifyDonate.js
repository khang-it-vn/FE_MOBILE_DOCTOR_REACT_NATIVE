...
export default function ScanVerifyDonate({ route, navigation }) {
  ...
  const getDataOfNHM = async (data) => {
    try {
      ...
      setIDSK(data.split("|")[1]);
      console.log("http://localhost:44369/api/BacSiRules/InforVerifyDonate?uid_guid="+data.split("|")[0]+"&idsk="+data.split("|")[1]);
      const res_info_data = await axios.get(
        "http://localhost:44369/api/BacSiRules/InforVerifyDonate?uid_guid="+data.split("|")[0]+"&idsk="+data.split("|")[1],
        config
    ...
    setTitleStateScanData("Scan mới")
     navigation.navigate("ScanQrVerifyDonate", {
