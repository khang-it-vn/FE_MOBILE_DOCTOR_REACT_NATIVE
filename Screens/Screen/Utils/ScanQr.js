...
export default function ScanQr({ route, navigation }) {
  ...
  const handleVerifyHealth = async () => {
    try {
      if(!theTich)
      {
        Alert.alert("System BloodBank", "Bạn cần chọn thể tích trước khi xác nhận");
        return;
      }
      let token = await AsyncStorage.getItem("token");
      token = "Bearer " + token;
      ...
      const res = await axios.patch(
        "http://localhost:44369/api/BacSiRules/VerifyHealth",
        {
          uid: data.uid,
          iD_SK: global.idsk,
          id_LTT: theTich,
    ...
    <View style={styles.container}>
      <View>
        ...
        <View>
          ...
          <View style={styles.nameComp}>
          <Text style={styles.name}>{loaiMau == null ? "Chưa xác định" : loaiMau.tenLoai}</Text>
        </View>
          <View>
            <SelectDropdown
              buttonStyle={{
