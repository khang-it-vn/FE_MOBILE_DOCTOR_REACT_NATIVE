...
export default function Scan({ route, navigation }) {
  ...
  const getDataOfNHM = async (data) => {
    try {
      ...
      console.log("http://localhost:44369/api/BacSiRules/InforVerifyHealth?uid_guid=" + data +"&idsk="+global.idsk);
      const res_info_data = await axios.get(
        "http://localhost:44369/api/BacSiRules/InforVerifyHealth?uid_guid=" + data +"&idsk="+global.idsk,
        config
        ...
        if (error.response) {
          ...
          {
            setTitleStateScanData("Không tìm thấy dữ liệu người dùng đăng ký\n Bạn có muốn Scan lại không?")
          }
